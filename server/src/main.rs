...
async fn main() -> anyhow::Result<()> {
    ...
    let router = worker
        .create_router(RouterOptions::new(vec![RtpCodecCapability::Audio {
            ...
            parameters: RtpCodecParametersParameters::default(),
            rtcp_feedback: vec![RtcpFeedback::Nack, RtcpFeedback::NackPli, RtcpFeedback::TransportCc],
        }]))
        .await?;
...
async fn handle_command(
    ...
    match command {
        ...
        ClientCommand::CloseProducer { producer_id } => close_producer(request_id, room, tx, peer_id, producer_id).await?,
        ClientCommand::Ping => {
...
async fn join(
    ...
    while guard.peers.values().any(|p| p.name.eq_ignore_ascii_case(&name)) {
...
fn turn_ice_servers() -> Vec<Value> {
    ...
    vec![json!({
        "urls": ["turn:turn.biclex.ru:3478?transport=udp", "turn:turn.biclex.ru:3478?transport=tcp"],
...
async fn produce(
    ...
    let producer = transport
        .produce(ProducerOptions::new(media_kind, serde_json::from_value(params)?))
        .await?;
    ...
    if kind == "audio" {
        broadcast_except(&guard, id, ServerEvent::NewProducer { peer_id: id.clone(), producer_id, name });
    } else {
        broadcast_except(&guard, id, ServerEvent::ScreenProducerStarted { peer_id: id.clone(), producer_id });
    }
    Ok(())
}
...
async fn consume(
    ...
    let (transport, producer, router) = {
        ...
        let producer = guard
            ...
            .values()
            .find_map(|p| {
                p.producer.as_ref().filter(|x| x.id().to_string() == producer_id).cloned()
                    .or_else(|| p.screen_producer.as_ref().filter(|x| x.id().to_string() == producer_id).cloned())
            })
...
async fn close_producer(
    ...
    let mut guard = room.lock().await;
    let id = peer_id.as_ref().ok_or_else(|| anyhow::anyhow!("join required"))?;
    let closed = {
        let peer = guard.peers.get_mut(id).ok_or_else(|| anyhow::anyhow!("peer not found"))?;
    ...
    if closed {
        broadcast_except(&guard, id, ServerEvent::ScreenProducerStopped { peer_id: id.clone(), producer_id });
    }
    reply(tx, request_id, ServerEvent::Pong);
    Ok(())
}
...
async fn cleanup_peer(room: &Arc<Mutex<Room>>, id: &str) {
    ...
    if let Some(mut p) = guard.peers.remove(id) {
        ...
        if let Some(producer) = p.screen_producer.take() {
            let pid = producer.id().to_string();
            drop(producer);
            broadcast_except(&guard, id, ServerEvent::ScreenProducerStopped { peer_id: id.to_owned(), producer_id: pid });
        }
        p.consumers.clear();
