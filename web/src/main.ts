...
versionButton.textContent = "Версия клиента: 0.1.36";
...
function showRoom(show: boolean) {
  ...
  roomPage.style.display = show ? "flex" : "none";
}
function updateJoinButton() {
  joinButton.disabled = !nameInput.value.trim() || joining;
}
...
async function connect(reconnecting = false, noMicrophone = false) {
  ...
  try {
    const signalingUrl = localSignaling
      ? "ws://10.70.0.50:8123/ws"
      : (import.meta.env.VITE_SIGNALING_URL ?? "ws://127.0.0.1:8123/ws");
    ...
    failedSocket?.close(1011, "connection setup failed");
